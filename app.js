...
function saveMoney(e,type){e.preventDefault();db.transactions.unshift({id:uid(),type,amount:+mAmount.value,note:mNote.value,date:new Date().toISOString()});save();closeModal();refresh();toast("Tersimpan")}
...
function saveProduct(e,id){e.preventDefault();let p=id?db.products.find(x=>x.id===id):{id:uid()};Object.assign(p,{name:pName.value,cat:pCat.value,cost:+pCost.value,sell:+pSell.value,stock:+pStock.value,min:+pMin.value,code:pCode.value.trim()});if(!id)db.products.unshift(p);save();closeModal();refresh();toast("Produk tersimpan")}
...
function addStock(e,id){e.preventDefault();let p=db.products.find(x=>x.id===id);p.stock+=+stockAdd.value;save();closeModal();refresh();toast("Stok ditambah")}
...
function manualCode(){let code=prompt("Masukkan barcode / QR:");if(!code)return;let p=db.products.find(x=>x.code===code);closeModal();if(p){saleForm();setTimeout(()=>sProduct.value=p.id,100)}else{productForm();setTimeout(()=>pCode.value=code,100)}}
...
function saveDebt(e){e.preventDefault();db.debts.unshift({id:uid(),type:dType.value,name:dName.value,amount:+dAmount.value,note:dNote.value,paid:false,date:new Date().toISOString()});save();closeModal();actions("debt");toast("Dicatat")}
...
function addCategory(e){e.preventDefault();let x=newCat.value.trim();if(x&&!db.categories.includes(x))db.categories.push(x);save();closeModal();actions("categories")}
...
function renderHome(){
 ...
 todayProfit.textContent=rupiah(profit);todaySales.textContent="Penjualan "+rupiah(sales);todayExpense.textContent="Pengeluaran "+rupiah(expense);todayTx.textContent=today.filter(t=>t.type==="sale").length;productCount.textContent=db.products.length;customerCount.textContent=db.customers.length;lowStock.textContent=db.products.filter(p=>p.stock<=p.min).length;
 let low=db.products.filter(p=>p.stock<=p.min).slice(0,5);lowStockList.innerHTML=low.length?low.map(p=>`<div class="item"><div><b>${esc(p.name)}</b><div class="item-meta">${esc(p.cat)}</div></div><b class="warn">${p.stock} tersisa</b></div>`).join(""):"<div class='item'><div class='item-meta'>Semua stok masih aman 🎉</div></div>";drawChart();
}
